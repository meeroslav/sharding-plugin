      ...
      return await createNodesFromFiles(
        async (configFilePath, options, context, idx) => {
          ...
          targetsCache[hash] ??= await buildJestTargets(
            configFilePath,
            projectRoot,
            options as JestPluginOptionsNormalized,
            context,
            presetCache
          );
...
async function buildJestTargets(
  configFilePath: string,
  projectRoot: string,
  options: JestPluginOptionsNormalized,
  ...
  if (disableJestRuntime) {
    ...
    if (options?.ciTargetName) {
      ...
      const specs =
        jestVersion >= 30
          ? await source.getTestPaths(config.globalConfig, config.projectConfig as any)
...
async function getJestOption<T = any>(
  ...
  if (rawConfig.preset) {
    ...
    try {
      let preset: any = presetCache[presetPath];
      if (!preset) {
